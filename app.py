...
def start(message):
    bot.send_message(message.chat.id, "به ربات Dr.ERROR خوش اومدي")
...
def bomb(message):
    ...
    bot.send_message(message.chat.id, "شماره بده تا بگامش:")
...
def handle_message(message):
    ...
    if chat_id in user_sessions and user_sessions[chat_id] == "waiting_phone":
        ...
        if phone in blocked_numbers:
            bot.send_message(chat_id, "چي فک کردي عبو سوفيان ؟؟")
